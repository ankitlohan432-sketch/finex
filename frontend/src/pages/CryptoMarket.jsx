...
import { TrendingUp, TrendingDown, RefreshCw, ChevronDown, Bitcoin } from 'lucide-react'
...
export default function CryptoMarket() {
  ...
  const fmt = (n, currency='USDT') => {
    if (!n || n === 0) return '—'
    const prefix = currency === 'USDT' ? '$' : '₹'
    if (n >= 1e9) return `${prefix}${(n/1e9).toFixed(2)}B`
    if (n >= 1e6) return `${prefix}${(n/1e6).toFixed(2)}M`
    if (n >= 1e3) return `${prefix}${n.toLocaleString('en-US', {minimumFractionDigits:2,maximumFractionDigits:2})}`
    return `${prefix}${Number(n).toFixed(n < 0.01 ? 6 : 2)}`
  }
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:20 }}>
        ...
        <span style={{ fontSize:12, color:'var(--text-muted)', background:'var(--bg-card-high)', padding:'2px 8px', borderRadius:20 }}>via Binance</span>
      ...
      <div className="grid-2-1">
        ...
        <div className="card" style={{ padding:0 }}>
            ...
            <table style={{ width:'100%' }}>
              <thead>
                <tr>
                  ...
                  <th style={{ padding:'12px 16px', textAlign:'right' }}>24h High</th>
                  <th style={{ padding:'12px 16px', textAlign:'right' }}>24h Low</th>
                  <th style={{ padding:'12px 16px', textAlign:'right' }}>Volume</th>
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  <tr><td colSpan={6} style={{ textAlign:'center', padding:40, color:'var(--text-muted)' }}>Loading crypto data...</td></tr>
                ) : tickers.map((t, i) => (
                  <tr key={t.binance_symbol || i}
                    ...
                    <td style={{ padding:'10px 16px' }}>
                      <div style={{ fontWeight:600, color:'var(--text-primary)', fontFamily:'monospace' }}>{t.symbol}</div>
          ...
          {hasMore && !loading && (
            <div style={{ padding:'12px 16px', textAlign:'center', borderTop:'1px solid var(--border-light)' }}>
              <button className="btn btn-ghost btn-sm" onClick={() => loadTickers(false)} disabled={loadingMore}
                style={{ gap:6 }}>
                <ChevronDown size={14} />
        ...
        <div>
          {selected ? (
            <>
              {/* Prediction Card */}
              <PredictionCard symbol={selected.binance_symbol} market="crypto" />
              ...
              <div style={{ display:'flex', gap:4, marginBottom:12, flexWrap:'wrap' }}>
                {INTERVALS.map(intv => (
                  <button key={intv} className={`btn btn-ghost btn-sm mono`}
                    style={{ padding:'3px 8px', fontSize:11, background: interval===intv ? 'var(--accent-light)' : undefined, color: interval===intv ? 'var(--accent)' : undefined, borderColor: interval===intv ? 'var(--accent)' : undefined }}
              ...
              <div style={{ position:'relative', minHeight:320 }}>
                ...
                {!scriptLoaded && <div style={{ height:320, display:'flex', alignItems:'center', justifyContent:'center', color:'var(--text-muted)' }}>Loading chart library...</div>}
              </div>
            ...
            <div className="card" style={{ textAlign:'center', padding:'60px 20px' }}>
              ...
              <p style={{ color:'var(--text-muted)', fontSize:13 }}>Select a coin to view candlestick chart</p>
            </div>
          )}
        </div>
      </div>
